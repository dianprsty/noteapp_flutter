  ...
  Stream<NoteState> mapEventToState(NoteEvent event) async* {
    ...
    if (event is LoadNotes) {
      try {
        ...
        final List<Map<String, dynamic>> data = await _databaseHelper.readAllNotes();
        // Mengonversi data yang diambil menjadi objek Note.
        final notes = data.map((note) => Note.fromMap(note)).toList();
        debugPrint('Loaded notes: $notes'); // Untuk debugging, menampilkan catatan yang telah dimuat.
        yield NotesLoaded(notes); // Menghasilkan state NotesLoaded yang berisi list notes.
      } catch (e) {
        yield NotesError("Failed to load notes: $e"); // Jika terjadi error, menghasilkan state NotesError.
      }
    }
    // Jika event adalah AddNote, kita akan menambahkan catatan baru ke database.
      ...
      try {
        ...
        if (state is NotesLoaded) {
          ...
          yield NotesLoaded(updatedNotes); // Menghasilkan state NotesLoaded yang diperbarui.
        }
      } catch (e) {
        yield NotesError("Failed to add note: $e"); // Jika terjadi error, menghasilkan state NotesError.
      }
    }
    // Jika event adalah UpdateNote, kita akan memperbarui catatan di database.
      ...
      try {
        ...
        yield NotesError("Failed to update note: $e"); // Jika terjadi error, menghasilkan state NotesError.
      }
    }
    // Jika event adalah DeleteNote, kita akan menghapus catatan dari database.
      ...
      try {
        ...
        yield NotesError("Failed to delete note: $e"); // Jika terjadi error, menghasilkan state NotesError.
      }
    }
  }
}
